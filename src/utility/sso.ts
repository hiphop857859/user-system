...
const sso = {
  validate (payload, sig) {
    ...
    return crypto.sha256(str) === sig
  },
  getNonce (payload) {
    const q = querystring.parse(
      new Buffer(querystring.unescape(payload), 'base64').toString() //NOSONAR
    )
    if ('nonce' in q) {
      return q.nonce
    } else {
  ...
  buildLoginString (params) {
    ...
    const payload = new Buffer(querystring.stringify(params), 'utf8').toString('base64') //NOSONAR
